...
| **containerId**        | String     | \_map.svgId         |                                                                                                                                                                                                                                                                                                                                            | 'map'\_ | The ID of the tooltips 'prison' element. Eurostat-map will ensure that the tooltip does not leave the bounds of this element. |
...
| **textFunction**       | Function   | _see example above_ | A function returning the html to show in a tooltip which appears when the mouse passes over map features. The function signature is `function(rg, map)` where `rg` is the selected region and `map` is the map. Set to _null_ if no tooltip is needed.                                                                                     |
