 ...
 */
export function createSqrtScale(rawData = [], maxSize = 20, minSize = 0) {
    // ----------------------------
    // 1. Sanitize data
    // ----------------------------
    const positives = rawData.map(Number).filter((v) => Number.isFinite(v) && v > 0)
    ...
    // ----------------------------
    const base = scaleSqrt().domain([0, dataMax]).range([0, maxSize]).clamp(true)
 ...
 * @param {number} minSize         Minimum visible radius for positive values
 */
export function createRadialScale(rawData = [], maxSize = 20, minSize = 0) {
    const positives = rawData.map(Number).filter((v) => Number.isFinite(v) && v > 0)
    ...
    const base = scaleRadial().domain([0, dataMax]).range([0, maxSize]).clamp(true)
 ...
 */
export function createLinearScale(rawData = [], maxSize = 20, minSize = 0) {
    // ----------------------------
    // 1. Sanitize data
    // ----------------------------
    const positives = rawData.map(Number).filter((v) => Number.isFinite(v) && v > 0)
    ...
    // 2. Base linear scale
    // ----------------------------
    const base = scaleLinear().domain([0, dataMax]).range([0, maxSize]).clamp(true)
