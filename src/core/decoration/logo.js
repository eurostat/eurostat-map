...
const getMapTopOffset = function (out) {
    if (!out.header_ || out.isInset) return 0
    ...
    return hb ? hb.height + headerPadding : 0
}

const getDefaultBottomY = function (out, elementHeight, margin) {
    return getMapTopOffset(out) + out.height_ - elementHeight - margin
}
...
export const addEurostatLogo = function (out) {
    ...
    if (group.empty()) {
        group = svg.append('g').attr('id', 'em-logo-group').attr('class', 'em-logo-group')
    }

    const logoWidth = out.logoWidth_ || 200
    const logoHeight = out.logoHeight_ || 40

    const x = out.logoPosition_ ? out.logoPosition_[0] : 4
    const y = out.logoPosition_ ? out.logoPosition_[1] : getDefaultBottomY(out, logoHeight, 0) + 2
...
export const addEurostatRibbon = function (out) {
    ...
    let x = out.ribbonPosition_ ? out.ribbonPosition_[0] : out.width_ - 124
    let y = out.ribbonPosition_ ? out.ribbonPosition_[1] : getDefaultBottomY(out, ribbonHeight, margin) + 2
