...
function buildSegments(out) {
    ...
    if (out.flowBidirectional_) {
        ...
        for (const link of links) {
            ...
            const sourceNode = nodeMap.get(sid) || (typeof link.source === 'object' ? link.source : null)
            const targetNode = nodeMap.get(tid) || (typeof link.target === 'object' ? link.target : null)
...
function drawSegments(out, container, segments, gradientIds, arrowIds) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (out.flowWidthGradient_) {
            paths = drawTaperedSegment(out, lineGroup, s, paint, colorKey, baseColor)
        } else {
            paths = drawStraightSegment(out, lineGroup, s, paint, colorKey, baseColor)
        }
...
function drawTaperedSegment(out, lineGroup, s, paint, colorKey, baseColor) {
    const dx = s.x2 - s.x1,
        dy = s.y2 - s.y1
...
function drawStraightSegment(out, lineGroup, s, paint, colorKey, baseColor) {
    const backoff = out.flowArrows_ ? arrowBackoffPxForStroke(s.width, out) : 0

    let x2 = s.x2,
        y2 = s.y2
    if (out.flowArrows_ && backoff > 0) {
        const dx = s.x2 - s.x1,
            dy = s.y2 - s.y1
        const len = Math.hypot(dx, dy) || 1
        x2 = s.x2 - (dx / len) * backoff
        y2 = s.y2 - (dy / len) * backoff
    }
...
export function getFlowStroke(out, originId, destId, route, halfValue) {
    ...
    if (typeof out.flowColor_ === 'function') {
        let color
        try {
            color = out.flowColor_(linkLike)
        } catch (_) {}
        if (color == null && out.flowColor_.length >= 3) {
            try {
                color = out.flowColor_(originId, destId, route)
            } catch (_) {}
        }
